...
5. **DIARY.md long-term game progression (MEM-04)**: Past sessions, accomplishments, and shared experiences accumulate in a chronological diary file that is loaded into every session's seed message.
   - Current: No file is written or read for long-term memory.
   - Target: `DIARY.md` exists at a configured path. Each active→idle transition appends a per-session summary (generated by one extra Anthropic call). Every N=10 active→idle transitions OR when `DIARY.md` exceeds a soft size cap (default 200 KB), a consolidation Anthropic call rewrites the older portion of the diary into a denser narrative. Both `OWNER.md` and `DIARY.md` are read at session-start and pasted into the seed user message under headings `# Owner` and `# Diary (recent first)`.
   - Acceptance: After three sessions ("chopped wood with SSk1tz", "fought a creeper", "explored a cave"), `DIARY.md` contains three dated entries newest-first. After 10 sessions, the consolidation call has fired exactly once, leaving fewer but denser entries. After bot restart, the next session's seed user message verifiably includes those entries (logged by the orchestrator).

6. **LLM-directed compaction trigger (MEM-02)**: Compaction (per-session summary + periodic consolidation) fires on the LLM's natural decision to terminate the active loop, not on a wall-clock timer.
   - Current: There are no compaction calls. The 10-second idle timer is a mechanical trigger but it does not write to disk.
   - Target: Per-session summary fires exactly once on `active→idle` transition (i.e. when the LLM returns a terminal response with no further tool_use). The 10-second idle probe never triggers compaction. Consolidation fires every N=10 per-session writes OR on size-cap pressure. The trigger is "the LLM decided to stop", which is a semantic boundary even though the orchestrator implements it; this satisfies MEM-02's intent ("decided by the personality LLM, not a mechanical timer").
   - Acceptance: Across a 60-minute session with 5 chat-driven active sessions and many 10-second idle ticks, the diary file is appended exactly 5 times (once per session terminal). No idle-tick-driven writes occur.
...
- [ ] **A1**: Running a session that emits multiple tool_uses across multiple iterations produces a `messages` array where every `tool_use` block has a paired `tool_result` block in the immediately-following user turn.
- [ ] **A2**: The 10-second idle probe NEVER fires while an active session is in flight (verifiable via logs).
- [ ] **A3**: An owner chat sent mid-action aborts the in-flight session, preserves its `messages` history, appends `PLAYER INTERRUPT: <text>` as a new user turn, and the next iteration runs on the same array.
...
- [ ] **A5**: After 3 chat-driven sessions, `DIARY.md` contains exactly 3 newest-first entries with timestamps and short summaries. After 10 sessions, exactly one consolidation pass has run.
- [ ] **A6**: A bot started with no memory files boots cleanly; the first session's seed user message contains placeholder text (not a missing-file error).
- [ ] **A7**: Across a 60-minute test with 5 active sessions and many idle ticks, `DIARY.md` is appended exactly 5 times — never on idle ticks.
...
*Next step: `/gsd-discuss-phase 3` — implementation decisions (Session class shape, message-array snapshot trimming, OWNER.md/DIARY.md schema, compaction prompt design, etc.)*
